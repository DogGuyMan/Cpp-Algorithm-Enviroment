...
{
	if (lhs.size() == 1) { BigIntMulSmall(rhs, lhs[0], out); return;}
	if (rhs.size() == 1) { BigIntMulSmall(lhs, rhs[0], out); return;}
	if (lhs.size() < rhs.size()) { BigIntMul(rhs, lhs, out); return;}
	...
	for (int i = 0; i < (int)rhs.size(); ++i)
	{
		...
		for (int j = 0; j < i; ++j) shifted.push_back(0);
...
// ull Combination(int r, int n) {
// 	ull nfac = Factorial(n);
// 	ull rfac = Factorial(r);
// 	ull nmrfac = Factorial(n-r);
// 	cout << nfac << ' ' << rfac << ' ' << nmrfac << '\n';
...
void HandleInput(istream &ins)
{
	...
	for(ull i = 2; i <= 30; i++) {
		DP[i][0] = {1};
		DP[i][i] = {1};
		BigIntAssign(i, DP[i][1]);
		BigIntAssign(i, DP[i][i-1]);
	}

	for(ull i = 3; i < 30; i++) {
		for(ull j = 1; j <= i; j++) {
			BigIntAdd(DP[i][j], DP[i][j+1], DP[i+1][j+1]);
		}
	}
